...
void pedirNombreRuta(char *ruta, char *nombre) {

    printf("Introduce la ruta del archivo de texto: ");
    scanf("%s", &ruta);
...
void mostrarAdaptadores() {
    ...
    char adaptador[100];
    printf("Ingrese el nombre del adaptador de red: ");
    fgets(adaptador, sizeof(adaptador), stdin);
    adaptador[strcspn(adaptador, "\n")] = '\0'; // Eliminar el salto de línea al final

    // Comando para obtener la configuración de red del adaptador
    char comando[200];
    sprintf(comando, "ipconfig /all | findstr \"%s\" > adaptador.txt", adaptador);

    // Ejecutar el comando
    system(comando);

    printf("La configuración de red del adaptador se ha guardado en el archivo adaptador.txt.\n");
}
